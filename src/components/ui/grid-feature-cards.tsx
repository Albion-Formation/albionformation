...
import { useId, type ComponentProps, type ComponentType, type SVGProps } from "react";
...
export function FeatureCard({ feature, className, ...props }: FeatureCardProps) {
  ...
  return (
    <div className={cn("relative overflow-hidden p-6", className)} {...props}>
      ...
      <h3 className="mt-10 text-sm font-semibold md:text-base">{feature.title}</h3>
      <p className="relative z-20 mt-2 text-xs font-light leading-relaxed text-muted-foreground md:text-sm">
        {feature.description}
      </p>
    </div>
  );
...
function GridPattern({
  ...
  squares,
  ...props
}: ComponentProps<"svg"> & { width: number; height: number; x: string; y: string; squares?: number[][] }) {
